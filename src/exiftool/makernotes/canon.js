...
exports.ref = {
	...
	CanonAperture: function(n){
		var v = (typeof n === 'number') ? Math.round(Math.exp( exports.ref.CanonEV(n)._val * Math.log(2)/2 ) * 100)/100 : n;
		return (typeof n === 'number') ? { value: v.toFixed(1) , _val: n } : { value: n , _val: n };
	},
	Selftimer: function(val){
		if( (typeof val === 'string' && val.indexOf('Off')>-1) || (typeof val === 'number' && val == 0) ) return { value:'Off', _val:0 };
		var v = (typeof val !== 'number') ? val : ((val&0xfff) / 10).toString().concat( ' s', ( (val&0x4000) ? ', Custom' : '') );
		return { value:v, _val:val }
	},
	PrintFocalRange: function(short, long, scale){
		if (typeof scale !== 'number') scale = 1;
		if (short === long) {
			return (short * scale).toFixed(1).concat(' mm');
		} else {
			return (short * scale).toFixed(1).concat(' - ', (long * scale), ' mm');
		}
	},
	...
	*/
	...
	FileInfo: function(arr, model){
		...
		var fi = {
			1: { desc: 'FileNumber', fn: function(){ return exports.ref.FileNumber(arr[1], model); } },
			
			3: { desc: 'BracketMode', values: {0:'Off',  1:'AEB',  2:'FEB',  3:'ISO',  4:'WB'} },
			4: { desc: 'BracketValue' },
			5: { desc: 'BracketShotNumber' },
			6: { desc: 'RawJpgQuality', values: (function(){ return exports.ref.Quality })() },
			7: { desc: 'RawJpgSize', values: (function(){ return exports.ref.Size })() },
	...
	CameraSettings: function(arr){
		var cs = {
			...
			*/
			...
			2: { desc: 'SelfTimer', fn:function(){ return exports.ref.Selftimer(arr[2]) }  },
			
			3: { desc: 'Quality', values: (function(){ return exports.ref.Quality })() },
			...
			10: { desc: 'CanonImageSize', values: (function(){ return exports.ref.Size })() },
			...
			22: { desc: 'LensType', fn:function(){ 
					var lt = (function(){ return exports.ref.LensType })();
					
					if(arr[22] in lt){
						if( typeof lt[arr[22]] === 'object' ){
							var l = { value: lt[arr[22]]._, _val:arr[22] };
							var key = Math.round(arr[23]+arr[24]+(exports.ref.CanonAperture(arr[26]).value*10));
			...
			26: { desc: 'MaxAperture', fn: function(){ return exports.ref.CanonAperture(arr[26]) } },
			27: { desc: 'MinAperture', fn: function(){ return exports.ref.CanonAperture(arr[27]) } },
	...
	ShotInfo: function(arr){
		var si = {
			...
			4: { desc:'TargetAperture', fn: function(){ return exports.ref.CanonAperture(arr[4]) } },
			
			5: { desc:'TargetExposureTime', fn: function(){ 
				if(typeof arr[5] !== 'number' || arr[5]==-32768 || arr[5]==32768) return { value:'n/a', _val:0 };
				return MainRef.expoTime( Math.exp( -Math.abs(exports.ref.CanonEV(arr[5])._val ) * Math.log(2) ) ) 
			} }, 
			
			6: { desc:'ExposureCompensation', fn: function(){ 
				return MainRef.decToFrac(exports.ref.CanonEV(arr[6])._val);
			} },
			 
			7: { desc: 'WhiteBalance', fn: function(){ 
					return (arr[7] in exports.ref.WhiteBalance) ? {value:exports.ref.WhiteBalance[arr[7]], _val:arr[7], priority:true} : {value:arr[7], _val:arr[7]};
				}
			},
			...
			15: { desc: 'FlashExposureComp', fn: function(){ 
				return MainRef.decToFrac(exports.ref.CanonEV(arr[15])._val) 
			} }, 
			
			16: { desc: 'AutoExposureBracketing', values: {_:'On',0:'Off', 1:'On (shot 1)', 2:'On (shot 2)', 3:'On (shot 3)'} },
			 
			17: { desc: 'AEBBracketValue', fn: function(){ return MainRef.decToFrac(exports.ref.CanonEV(arr[17])._val) } }, 
			...
			21: { desc: 'FNumber', fn: function(){ return exports.ref.CanonAperture(arr[21]) } },
			...
			*/
			...
			29: { desc: 'SelfTimer', fn:function(){ return exports.ref.Selftimer(arr[29]) } },
			33: { desc: 'FlashOutput' }
		};
	...
	ProcessingInfo: function(arr){
		var pr = {
			...
			8: { desc: 'WhiteBalance', values: (function(){ return exports.ref.WhiteBalance })() },
			9: { desc: 'ColorTemperature' },
			10: { desc: 'PictureStyle', values: (function(){ return exports.ref.PictureStyle })() },
	...
	ModifiedInfo: function(arr){
		var mo = { 
			...
			8: { desc: 'ModifiedWhiteBalance', values:(function(){ return exports.ref.WhiteBalance })() }, 
			9: { desc: 'ModifiedColorTemp' },
			10: { desc: 'ModifiedPictureStyle', values:(function(){ return exports.ref.PictureStyle })() }, 
...
exports.postFn = function(res){
	...
	var composite = {
		DriveMode: function(){
			var vals = ['ContinuousDrive', 'SelfTimer'];
			var modes = ['Continuous Shooting', 'Self-timer Operation', 'Single-frame Shooting'];
			var key = (vals[0] in res && res[vals[0]]._val!=0) ? 0 : ((vals[1] in res && res[vals[1]]._val>0) ? 1 : 2);
			var val = (key>0 && (vals[1] in res)) ? res[vals[1]]._val : ((vals[0] in res) ? res[vals[0]]._val : 0);
			return { value: modes[key], _val: val };
		},
		Lens: function(){
			if ('MinFocalLength' in res && 'MaxFocalLength' in res) return exports.ref.PrintFocalRange(res['MinFocalLength'], res['MaxFocalLength']);
			return false;
		},
		/* TODO
		Lens35efl => {
		...
		*/
		ShootingMode: function(){
			var vals = ['CanonExposureMode', 'EasyMode', 'BulbDuration'];
			var val = (vals[0] in res) ? res[vals[0]].value : ((vals[1] in res) ? res[vals[1]].value : 0);
			var __v = (vals[0] in res) ? res[vals[0]]._val : ((vals[1] in res) ? res[vals[1]]._val : 0);
			if (vals[0] in res && vals[2] in res && res[vals[0]]._val===4){ 
				val = 'Bulb';
				__v = 7;
			}
			if (!(vals[0] in res) && vals[1] in res) __v = res[vals[1]]._val+10;
			return { value: val, _val: __v }
		},
		/* TODO (FlashBits)
			FlashType => {
				Notes => q{
		...
		*/
		WB_RGGBLevels: function(){
			var desire = ['WB_RGGBLevelsAsShot', 'WB_RGGBLevelsAuto', 'WB_RGGBLevelsDaylight', 'WB_RGGBLevelsCloudy', 'WB_RGGBLevelsTungsten',  'WB_RGGBLevelsFluorescent', 'WB_RGGBLevelsFlash', 'WB_RGGBLevelsCustom', 'WB_RGGBLevelsShade', 'WB_RGGBLevelsKelvin'];
			if (desire[0] in res) return res[desire[0]];
			if ('WhiteBalance' in res && key in res){
				var key = desire[res['WhiteBalance']._val+2];
				return res[key];
			}
			return false;
		},
		ISO: function(){
			if ( !('ISO' in res) ){
				var desire = ['CameraISO', 'BaseISO', 'AutoISO'];
				return Math.round(res[desire[0]] || res[desire[1]] || res[desire[2]]);
				return false;
			}	
		},
		DigitalZoom: function(){
			var req = ['ZoomSourceWidth', 'ZoomTargetWidth', 'DigitalZoom'];
			if ( req[0] in res && req[1] in res && req[2] in res && req[2] === 3 ) return res[req[1]]/res[req[2]];
			return false;
		},
		/* TODO 
		// this is binary data, e.g. found in 5D Mark III
		// AddOriginalDecisionData: 'On', OriginalDecisionData: ...
		...
		*/
		FileNumber: function(){
			var desire = ['DirectoryIndex', 'FileIndex'];
			return res[desire[0]] || res[desire[1]];
			return false;
		},
